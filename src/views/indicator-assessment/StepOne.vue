  ...
  <div class="step-one">
    ...
    <div class="upload-container">
      <div class="upload-header">
        ...
        <div class="new-solution-link" @click="handleAddUploadArea">
          <img src="@/assets/assess/upload.png" alt="上传" class="icon-plus" />
          <span class="link-text">新增方案</span>
        </div>
      </div>
      <div class="upload-content">
        <div class="upload-areas-container" ref="uploadAreasRef">
          <div 
            v-for="(upload, index) in uploadAreas" 
            :key="index"
            class="upload-area" 
            ...
            <div class="upload-content">
              <template v-if="upload.image">
                <div class="uploaded-image-container">
                  <img :src="upload.image" alt="已上传图片" class="uploaded-image" />
                  <div class="upload-overlay" @click.stop="handleDeleteImage(index)">
...
const props = defineProps({
  ...
  uploadAreas: {
    type: Array,
    default: () => [
      { image: null, file: null, title: '方案1' },
      { image: null, file: null, title: '方案2' }
    ]
  },
  designType: {
    type: String,
...
const emit = defineEmits(['update:functionName', 'update:functionDesc', 'update:subjectCount', 'update:selectedIndicators', 'update:roleList', 'update:uploadedImage', 'update:originalFile', 'update:uploadedImages', 'update:uploadAreas', 'update:designType', 'next'])
...
const uploadAreas = computed({
  get: () => props.uploadAreas,
  set: (value) => emit('update:uploadAreas', value)
})
...
// 添加一个上传区域
const handleAddUploadArea = () => {
  uploadAreas.value.push({ image: null, file: null, title: `方案${uploadAreas.value.length + 1}` })
}
...
const handleFileChange = (event, index) => {
  ...
  if (file) {
    ...
    // 保存原始文件对象
    uploadAreas.value[index].file = file
    
    // 创建文件预览
    if (uploadAreas.value[index].image) {
      URL.revokeObjectURL(uploadAreas.value[index].image)
    }
    uploadAreas.value[index].image = URL.createObjectURL(file)
    
    // 设置默认方案名称（如果没有设置）
    if (!uploadAreas.value[index].title) {
      uploadAreas.value[index].title = `方案${index + 1}`
    }
    
    // 更新uploadedImages数组
    const allImages = [...uploadedImages.value]
    // 检查是否需要更新现有图片或添加新图片
    if (index < allImages.length) {
      allImages[index] = uploadAreas.value[index].image
    } else {
      allImages.push(uploadAreas.value[index].image)
    }
    uploadedImages.value = allImages
    
    // 如果是第一个上传框，更新兼容性属性
    if (index === 0) {
      originalFileModel.value = file
      uploadedImageModel.value = uploadAreas.value[0].image
    }
...
const handleDeleteImage = (index) => {
  // 如果是第一个上传框，并且是唯一的图片来源
  if (index === 0 && uploadAreas.value[0].image === uploadedImageModel.value) {
    // 清除兼容性属性
  ...
  // 清空上传区域
  uploadAreas.value[index].image = null;
  uploadAreas.value[index].file = null;
  // 保留方案标题，不重置
...
const handleMaterialSelect = (material) => {
  ...
  if (material && material.image) {
    // 查找第一个没有图片的上传框
    const emptyAreaIndex = uploadAreas.value.findIndex(area => !area.image);
    let targetIndex;
    
    if (emptyAreaIndex !== -1) {
      // 如果找到空的上传框，使用它
      targetIndex = emptyAreaIndex;
    } else {
      // 如果所有框都已有图片，添加新框
      uploadAreas.value.push({ image: null, file: null, title: `方案${uploadAreas.value.length + 1}` });
      targetIndex = uploadAreas.value.length - 1;
    }
    
    // 设置上传区域的图片
    uploadAreas.value[targetIndex].image = material.image;
    
    // 设置方案标题，使用素材标题或默认标题
    if (material.title) {
      uploadAreas.value[targetIndex].title = material.title;
    } else if (!uploadAreas.value[targetIndex].title) {
      uploadAreas.value[targetIndex].title = `方案${targetIndex + 1}`;
    }
    ...
    if (targetIndex < allImages.length) {
      allImages[targetIndex] = material.image;
    } else {
      allImages.push(material.image);
    }
    uploadedImages.value = allImages;
...
const handleNextStep = () => {
  ...
  // 验证图片是否上传
  if (uploadAreas.value.every(area => !area.image)) {
    errors.value.uploadedImage = true
    isValid = false
    firstErrorElement = uploadAreasRef.value
  }
...
const handleBatchSettings = () => {
  // 记录当前页面状态
  localStorage.setItem('returnToIndicatorAssessment', 'true')
  // 跳转到角色库页面
...
// 在组件挂载时，如果有原始文件但没有图片URL，重新创建图片URL
onMounted(() => {
  // 重新创建图片URL
  if (originalFileModel.value && !uploadedImageModel.value) {
    ...
    // 同时更新第一个上传区域
    if (!uploadAreas.value[0].image) {
      uploadAreas.value[0].image = uploadedImageModel.value
      uploadAreas.value[0].file = originalFileModel.value
    }
  }
  ...
  // 检查是否从角色库返回
  else if (localStorage.getItem('returnToIndicatorAssessment') === 'true') {
    // 清除标记
...
watch(() => uploadAreas.value, (newVal) => {
  if (newVal.some(area => area.image)) {
...
.upload-container {
  ...
  .upload-area {
    flex: 0 0 calc(50% - 8px); /* 两个上传框在一行，考虑间距 */
    min-width: 200px;
    min-height: 160px;
    border-radius: 6px;
    position: relative;
...
.error-highlight {
  animation: errorHighlight 0.6s ease;
}

@keyframes errorHighlight {
  0% {
    box-shadow: 0 0 0 0 rgba(245, 99, 77, 0.5);
  }
  70% {
    box-shadow: 0 0 0 10px rgba(245, 99, 77, 0);
  }
  100% {
    box-shadow: 0 0 0 0 rgba(245, 99, 77, 0);
  }
}
...
.upload-area.error {
  border: 1px dashed #F5634D !important;
}
